...
public class BitIteratorInt implements Iterator<Integer> {
	...
	private BitSequence.DIRECTION direction;
	...
	public BitIteratorInt(byte[] byteArr, int bitCount, BitSequence.DIRECTION direction) {
	...
	private void init() {
		...
		if(direction == BitSequence.DIRECTION.LEFT_TO_RIGHT){
			currentBitIndex = -1;
			currentByteIndex = 0;
